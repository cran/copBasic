...
function(u,v,...) {
  if(length(u) > 1 & length(v) > 1 & length(u) != length(v)) {
    ...
    warning("longer object length is not a multiple of shorter object length, no recycling in PSP()")
    return(NA)
  }
  ...
  return(sapply(1:length(u), function(i) { (u[i]*v[i])/(u[i] + v[i] - (u[i]*v[i])) }))
}
