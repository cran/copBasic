"isCOP.PQD" <- function(cop=NULL, para=NULL, uv=NULL, empirical=FALSE, verbose=TRUE, ...) {
      ...
      if(empirical) { # sample versions
         ...
         if(verbose & ! is.null(uv)) message("      Gini's Gamma = ",round(gin, digits=4))

         rho <- cor(U, V, method="spearman")
         if(verbose & ! is.null(uv)) message("    Spearman's Rho = ",round(rho, digits=4))

         tau <- cor(U, V, method="kendall")
         if(verbose & ! is.null(uv)) message("     Kendall's Tau = ",round(tau, digits=4))
       } else {
